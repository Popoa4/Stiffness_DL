...
# /Users/ethanshao/Desktop/ucl/research project/CNN/CNN+LSM/Dataset
...
def validation(model, device, optimizer, test_loader, mean_sigma):
    ...
    with torch.no_grad():
        for X, y_scale, y in test_loader:
            ...
            pred = (output.detach().cpu().numpy() * sigma + mean).reshape(-1)
            y = y.reshape(-1)
    ...
    torch.save(model.state_dict(), os.path.join(save_model_path, 'model{}.pth'.format(epoch + 1)))
